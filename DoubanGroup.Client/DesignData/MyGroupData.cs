...
namespace DoubanGroup.Client.DesignData
{
    public class MyGroupData
    {
        ...
        {
            public List<Group> JoinedGroupList { get; set; }
        }
        ...
        public MyGroupData()
        {
            this.CurrentUser = new CurrentUserData();
            this.CurrentUser.JoinedGroupList = new List<Group>();

            for (var i = 0; i < 20; i++)
            {
                this.CurrentUser.JoinedGroupList.Add(new Group
                {
                    Name = "成都豆瓣成都豆瓣成都豆瓣",
